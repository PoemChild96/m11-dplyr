...
seattle.flights <- data.frame(filter(flights, dest == "SEA"))
View(seattle.flights)

# On average, did flights to seatac gain or loose time?
mean(seattle.flights$gained, na.rm = TRUE)
...
Specific <- function(ori, des, int) {
  return (flights %>% filter(origin == ori, dest == des) %>% select_(int))
}

# Retireve the air_time column for flights from JFK to SEA
jfk.to.sea <- Specific("JFK", "SEA", "air_time")
